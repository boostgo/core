// Package worker is job/worker implementation. Based on time.Ticker.
// Features:
// - Stop when global context canceled (appx).
package worker
...
type Action func(ctx context.Context, logger log.Logger) error
type Middleware func(ctx context.Context, logger log.Logger) error
...
func (worker *Worker) runAction() error {
	logger := log.Namespace(worker.name)
	...
	if err := errorx.TryContext(ctx, func(ctx context.Context) error {
		...
		for _, middleware := range worker.beforeMiddlewares {
			...
			if err := middleware(ctx, logger); err != nil {
		...
		return worker.action(ctx, logger)
	}); err != nil {
		log.
